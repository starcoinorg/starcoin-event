...
function decodeSourceData() {
  const provinces = sourceData['86'];
  Object.keys(provinces).forEach((provinceKey) => {
    const cities = sourceData[provinceKey];
    const citiesArr = [];
    Object.keys(cities).forEach((cityKey) => {
      const counties = sourceData[cityKey];
      const countyArr = [];
      Object.keys(counties).forEach((countyKey) => {
        countyArr.push({
          label: counties[countyKey],
          value: counties[countyKey],
        });
      });
      citiesArr.push({
        label: cities[cityKey],
        value: cities[cityKey],
        children: countyArr,
      });
    });
    ret.push({
      label: provinces[provinceKey],
      value: provinces[provinceKey],
      children: citiesArr,
    });
  });
}
